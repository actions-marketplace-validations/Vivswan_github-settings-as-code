...
One line in a fleet's baseline can delete labels on two hundred repositories. Check mode on pull requests is the plan step, and it runs on the same rendered documents the apply will run.

Three jobs. `plan` lists the files in the flat `.github/repos/` directory; `render` folds each repository's layers (no token needed) and uploads the rendered document; `preview` collects them into one directory and runs a single multi-repo check over it, so the pull request gets one comment:
